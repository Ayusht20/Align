...
export default function HomePage() {
  ...
  async function fetchPosts() {
    setLoading(true);
    try { 
      const r = await api.get(`/api/posts/?sort=${sort}`); 
      setPosts(r.data); 
    } catch (e) { 
    ...
    <div className="flex gap-6 items-start">
      ...
      <div className="w-72 shrink-0 hidden lg:block space-y-4 sticky top-20">
        <div className="rounded-2xl border overflow-hidden animate-slide-left"
          ...
          <div className="p-4">
            {/* Brand Icon */}
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-orange-400 to-pink-500 flex items-center justify-center text-white font-bold text-xl -mt-9 mb-3 shadow-lg animate-pop-in">
              A
            </div>
            ...
            <Link href="/submit" className="btn-primary block text-center mb-2 text-sm py-2">
              ✏️ Create Post
            </Link>
            <Link href="/communities/create" className="btn-secondary block text-center text-sm py-2 border rounded-xl">
              🏘 Create Community
            </Link>
          </div>
        </div>
